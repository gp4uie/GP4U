...
**Generated pages.** `index`, `online`, `book-now`, `fees`, `about` and `faq` are generated by `scripts/generate-pages.js`
(edit the text there, then `npm run generate`). The FAQ answers are defined once and feed the FAQ page, its search-engine
FAQ data, and the short FAQ lists on the homepage and Online GP page. **Do not hand-edit those six HTML files.**
`walk-in`, `services`, `new-patients`, `contact` and the older pages (condition pages, booking, blog, privacy…) are plain HTML.
