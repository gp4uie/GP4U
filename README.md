...
- **Doctor dashboard** (`/dashboard.html`) — password-protected, with three top-level tabs:
  ...
  - Onboarding doctors and setting anyone's working hours is **admin-only** now (see below) — a
    doctor logged into their own dashboard has no access to add/remove colleagues or change any
    schedule, their own included.
...
## 11. Site structure, shared parts and how to edit (September 2026 UX/UI upgrade)

**Navigation (every public page):** Walk-In Clinic · Online GP · Services · Fees · About · FAQs · Contact · **Book Now**.
"Patient login" (or "My account" once signed in) sits in the slim top bar with the emergency number. On phones a
sticky **Online GP / Walk-in** bar stays at the bottom of the screen.

**Booking journey:** *Book Now* (`book-now.html`) → choose **Online GP** (→ `book.html`, the 4-step wizard) or
**Walk-in** (→ `walk-in.html#book-in`). `book.html` states clearly that it is an online booking, not a clinic appointment.

**Shared parts — edit once, apply everywhere.** The header, footer, top bar, mobile booking bar and `<head>` links live in
`scripts/partials/`. After editing one, run:

    npm run pages        (or: node scripts/build-pages.js)
...
**Checks run before release:** layout sweep at 14 screen widths (320px–2560px, no horizontal overflow), JavaScript-error
sweep of every page, metadata/alt/label/landmark/link audit, colour-contrast check (WCAG AA), and end-to-end tests of the
...
### V3 pass — where things are configured, and what the owner still needs to confirm
...
- **Health Info** (`/blog.html`) is out of the navigation, footer and sitemap (and marked noindex) because its three starter posts are
  generic seed text, not GP-authored. Edit/replace them in the content editor, then add the link back to `scripts/partials/footer.html`.
- **Privacy notice — owner to confirm before relying on it legally:** the retention schedule (the page currently says only "as required
  by Irish law and Medical Council guidance"), the lawful bases under GDPR Articles 6 and 9, and the full list of service providers.
...
### Clinic documents (PDF and printable) and emails

- Prescriptions, sick certificates and referral letters are generated by `server/pdf.js` (emailed / **Download PDF**) and shown on `print-rx.html` / `print-doc.html`
  (Print). They share one letterhead and footer built from the clinic's details in Admin → Website settings (`server/practice.js`): name, tagline, address,
  phone, email, website, company details, a reference number (RX-/CERT-/REF-), issue date/time (Irish time) and a confidentiality line.
- Each shows the visit type and date (walk-in or online), the patient's address and allergies (filled from their clinic registration or portal profile when the
  booking has none — the usual case for walk-ins), the pharmacy (doctors can record it when issuing a prescription) and, on referrals, current medicines.
